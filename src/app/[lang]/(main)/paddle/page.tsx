...
export async function generateMetadata({ params }:{params: { lang: Locale }}) {
    return {
        alternates: { canonical: `https://imagedescriptiongenerator.xyz/${params.lang}/paddle` }
    }
  }

export default async function page({ params: { lang } }: { params: { lang: Locale } }) {
    const dictionary:Dictionary = await getDictionary(lang);
    const paddle_token = String(process.env.PADDLE_API_AUTH_TOKEN);
    const paddle_env = process.env.PADDLE_API_ENV ? Environment.sandbox : Environment.production; //'production' | 'sandbox'
                ...
                {dictionary.pricing.prices.map((price, index) => {
                    return (
                        <div key={index} className="flex flex-col w-full  p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <div>
                                <Button btnlabel={price.buy_lable} priceId={price.variantId} lang={lang} token={paddle_token} env={paddle_env}  email={email}/>
                            </div>
